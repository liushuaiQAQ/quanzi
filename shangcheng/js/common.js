$(function() {
	...
	$(document).on("click", ".publish_rticle .unfold", function() {
		$(this).parents(".content_items").find(".publish_rticle .article_content").css("max-height", "100%");
		$(this).html("收起");
		$(this).addClass("putflod");
	})
	$(document).on("click", ".publish_rticle .putflod", function() {
		$(this).parents(".content_items").find(".publish_rticle .article_content").css("max-height", "144px");
		$(this).html("展开");
			...
			$.ajax({
				...
				success: function(mssg) {
					if(mssg.status == 0) {
						...
						str += '<dd class="authorName"><b>' + (msg.nickname||shieldNumber(msg.username))+ '</b>';
						//用户等级   和会员等级   性别
						...
						if(msg.username != UserName) { //判断滑过自己的头像
							...
							if(msg.relation == 1 || msg.relation == 2 || msg.relation == 3) {
								str += '<li class="remark_in">设置备注名</li>' +
									'<li>推荐给好友</li>' +
									'<li class="complaint_in">举报</li>';
							} else {
								str += '<li class="complaint_in">举报</li>';
							}
							str += '</ul>' +
								'</div>' +
								'</a>' +
							...
							str += '<div class="userinformation_b"><p></p></div>';
						}
 ...
 * 评论
 * */
function createComment(productId, pid, content, comments_item, username) {
	$("#msk").remove();
	if($.trim(content) == "") {
	...
	$.ajax({
		...
		success: function(msg) {
			if(msg.status == 0) {
				...
				var mssg = msg.data;
				stairComment(mssg, comments_item, username);
 ...
 * 一级评论  和  二级评论
 * */
function stairComment(mssg, comments_item, username) {
	...
	if(!mssg.pid) {
		...
		str += '<span class="reply_text">回复</span>' +
			'<a href="javascript:;" class="qz_name reply_content" >' + mssg.repliedGuyName + '</a>' + toFaceImg(mssg.content) + '';
	}
			...
			if(k < 20) {
				...
				if(commentlist[k].pid == 0) {
					...
					if(commentlist[k].list) {
						...
						for(var ti = 0; ti < commentlist[k].list.length; ti++) {
							var result = commentlist[k].list[ti];
							if(result.pid == commentlist[k].id) {
								str += TwoList(result, plnumberof, commentlist);
							}
						}
					}
...
function TwoList(msg, numberof, commentlist) {
	if(plnumberof > 3) {
		...
		var two_comment = '<li class="comments-item HideComments" data-id="' + msg.id + '" data-name="'+msg.username+'"><div class="comments-item-bd">' +
			'<div class="ui-avatar">';
	} else {
		var two_comment = '<li class="comments-item" data-id="' + msg.id + '" data-name="'+msg.username+'"><div class="comments-item-bd">' +
			'<div class="ui-avatar">';
	}
	if(!msg.headProtrait) {
		...
		if(msg.headProtrait == "--") {
			...
			two_comment += '<img src="' + ImgHOST() + msg.headProtrait + '" />';
		}

	}
	two_comment += '</div>' +
		'<div class="comments-content">' +
		'<a href="javascript:;" class="qz_name" data-name=' + msg.username + '>' + (msg.nickName || shieldNumber(msg.username)) + '</a><span class="reply_text">回复</span>' +
		'<a href="javascript:;" class="qz_name reply_content" data-name=' + msg.repliedGuyName + '>' + msg.repliedGuyName + '</a>' + toFaceImg(msg.content) + '' +
		'<div class="comments-op">' +
		'<span class="ui-mr10 state">' + formatTime(msg.createtime) + '</span>' +
		'<a href="javascript:;" class="act-reply" title="回复"></a>';
	if(msg.username == UserName) {
	...
	for(var flyers = 0; flyers < commentlist.length; flyers++) {
		if(commentlist[flyers].pid == msg.id) {
			plnumberof++
 ...
 * 商品文章图片
 * */
function imgVideo(mssg, imagepath) {
	...
	if(imagepaths != 0) {
		...
		if(mssg.tcProduct.tcProducttype == 8) { //转发展示
			...
			if(window.location.href.indexOf("searchresult.html") > -1) {
				str += '<li class="specialOn specialOn_1" data-Surl="' + mssg.tcProduct.content.shareImageUrl + '" data-conType="' + mssg.tcProduct.content.shareType + '" data-content="' + mssg.tcProduct.content.shareTitle + '" class="Forwardpage" data-page="' + mssg.tcProduct.content.shareUrl + '">' +
					'<a class="clearfix" href="' + mssg.tcProduct.content.shareUrl + '">';
			} else {
				str += '<li class="specialOn specialOn_2" data-Surl="' + mssg.tcProduct.content.shareImageUrl + '" data-conType="' + mssg.tcProduct.content.shareType + '" data-content="' + mssg.tcProduct.content.shareTitle + '" class="Forwardpage" data-page="' + mssg.tcProduct.content.shareUrl + '">' +
					'<a class="clearfix" href="' + mssg.tcProduct.content.shareUrl + '">';
			};
			...
			if(url != undefined && url != "") {
				...
				if(mssg.tcProduct.content.shareImageType == 1) {
...
$(document).on("click", ".content_items .collection_post", function() {
	...
	var prdType = $(this).parents(".content_items").attr("data-prdtype");
	var _this = $(this);
	if(UserName) {
		createCollections(DeletePosts, prdType, _this);
	}
})

// 添加收藏
function createCollections(DeletePosts, type, _this) {
	$.ajax({
		type: "post",
		headers: {
			"token": qz_token()
...
$(document).on("click", ".content_items .collection_Cancel", function() {
	var prdId = $(this).parents(".content_items").attr("data-id"); //帖子编号
	var _this = $(this);
	// if ($('.joinUs').attr('data-status') == 0) {
	// 	warningMessage('请先加入圈子');
	// } else {
...
$(document).on("click", ".content_items .comments-op .oneselfdel", function() {
	var topicNo = $(this).parents(".comments-item").attr("data-id"); //帖子编号
...
if(UserName) {
	...
	$(document).on("click", ".content_items .review_1 .publish", function() {
		var productId = $(this).parents(".content_items").attr("data-id"); //帖子编号
		var pid = 0; //评论的父级评论编号
		...
		createComment(productId, pid, content, comments_item, username);
	})

	//发表二级评论
	$(document).on("click", ".content_items .revert_box .import .publish", function() {
		var productId = $(this).parents(".content_items").attr("data-id"); //同城产品id
		var pid = $(this).parents("li.comments-item").attr("data-id"); //父级id
 ...
 * 用户信息
 */
function publisherInfo(mssg, type) {
	var contentType = "";
	...
	var str = "";
	str = '<div class="content_items" data-type="' + contentType + '" data-id="' + mssg.tcProduct.id + '" data-name="' + mssg.user.username + '" data-prdType="' + mssg.tcProduct.type + '">' +
	...
	//				str += '<div class="shield"><a class="fl_menu" href="javascript:;"></a><ul class="fl_menu_list"><li class="shield_dt">屏蔽此动态</li><li class="shield_dt_all">不看他动态</li><li class="complaint">举报</li><ul></div>'
	//			}
 ...
 *	点赞人头像
 */
function publishBottom(mssg, clickMaplist, type) {
	var str = "";
	var from = "";
	var loca = ""
	//用户文章发布位置
	if(mssg.tcProduct.location) {
		var prdLocation=mssg.tcProduct.location.split("|");
		if(prdLocation[0].indexOf("市")>0&&prdLocation[1].indexOf("市")>0){
			prdLocation.splice(0,1);
		}
		prdLocation=prdLocation.join("-");
		str += '<div class = "location"><p><span>' +prdLocation+ '</span>|<span>' + mssg.tcProduct.productType + '</span></p></div>';
	}
	str += '</div><div class="qz_from">' + from + '</div>' +
		'<div class="qz_handle">' +
		'<ul class="qz_row_line">';
	if(username == mssg.user.username) {
		str += '<li>' +
			'<a class="again_edit" href="/shangcheng/publish/index.html?productId=' + mssg.tcProduct.id + '" ><span>去编辑</span></a>' +
			'</li>' +
			'<li style="padding-left: 0;" proId="' + mssg.tcProduct.id + '" >';
		if(mssg.tcProduct.isSale) {
		...
		str+='<li>';
		if(mssg.user.imgforheadlist && mssg.user.imgforheadlist[0]) {
			str += '<a class="fast_contact" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="' + ImgHOST() + mssg.user.imgforheadlist[0].imagepath + '" data-myindustry="' + mssg.user.myindustry + '">快速联系</a>';
		} else {
			str += '<a class="fast_contact" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="/img/first.png" data-myindustry="' + mssg.user.myindustry + '">快速联系</a>';
		}
		str += '</li>';
		//收藏
		if(mssg.favorite) {
		...
		str += '<li>|</li>' +
			'<li class="phone_box">' +
			'<a  class="contact_method" href="javascript:;"><i></i>联系方式</a>' +
			'<div class="contact_info">' +
			'<p>' + mssg.tcProduct.contacts + '</p>' +
			'<p>' + mssg.tcProduct.mobile + '</p>' +
			'<p>' + (mssg.tcProduct.otMobile || "") + '</p>' +
			'</div>' +
			'</li>' +
			'<li>|</li>' +
			'<li>' +
			...
			'<li class="complaint ' + no_login + '">举报</li>';
	}
	str += '<ul></ul>' +
		'</ul>' +
		'</li>' +
...
/*
 留言
*/
function commentsContent(mssg, commentlist, nums, str, cl) {
	str += '<div class="chitchatBox clear"><ul class="stairComment">';
	str += CommentOnContent(commentlist);
	if(commentlist) {
		if(commentlist.length > 20) {
			var ThereAre = commentlist.length - 20;
			str += '</ul><div class="To_view_more"><a href="/center/shopDetails.html?id=' + mssg.tcProduct.id + '" class="view_more">后面还有' + ThereAre + '条点击查看&gt;</a></div></div>';
		} else {
	...
	str += '</div>' +
		'<div class="p_input">' +
		'<textarea class="Postcomment" placeholder="写留言…" name="review_1"   /></textarea>' +
		'</div>' +
		'</div>' +
 ...
 *	商品文章内容
 */
function publishContent(mssg) {
	var str = "";
	var price = "";
	if(mssg.tcProduct.price) {
		price = priceUnit(mssg, "price");
	} else {
		if(mssg.tcProduct.originalPrice) {
			price = priceUnit(mssg, "originalPrice");
		} else {
			price = "面议";
		}
	}
	str += '<div class="article">' +
		'<p class="title"><a href="/center/shopDetails.html?id=' + mssg.tcProduct.id + '">' + mssg.tcProduct.title + '</a></p>' +
		'<p class="salary">' + price + '</p>';
	if(mssg.tcProduct.tcProducttype == 8) { //转发		
		...
		if(mssg.tcProduct.content && mssg.tcProduct.content.indexOf("\n") > -1) { //判断有否有回车符  有则换行
			var tcProductContent = mssg.tcProduct.content.replace(/\n/g, "</br>");
		} else {
			var tcProductContent = mssg.tcProduct.content;
		}
		str += '<div class="article_content" data-content="' + mssg.tcProduct.content + '">' + toFaceImg(tcProductContent) + '</div></div>';
	}

	str += '<div class="onDisplay"></div>' +
		'<div class="pictureShow photoblock-many">' +
		'<ul>';
	//tcProducttype==2  动态为视频
	if(mssg.tcProduct.videourl) {
...
//价格单位
function priceUnit(mssg, unitType) {
	var price = "";
	var rangeMoney = {
		0: "面议",
		1: "1000元以下",
	...
	if(mssg.tcProduct.type == 1 || mssg.tcProduct.type == 2) {
		if(unitType == "originalPrice") {
			if(mssg.tcProduct[unitType] == 1 || mssg.tcProduct[unitType] == 9) {
				price = rangeMoney[mssg.tcProduct[unitType]] + "/月";
			} else {
				price = rangeMoney[mssg.tcProduct[unitType]] + "元/月";
			}
		} else {
			price = mssg.tcProduct[unitType] + "元/月";
		}

	} else {
		price = "￥" + mssg.tcProduct.price + "元";
	}
	return price;
}
...
//商品下架
function commodityFrameFun(id,_this){
    $.ajax({
        type: "post",
