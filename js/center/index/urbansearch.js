...
$(document).on("click", ".Complaints_btn .ts_submit", function(e) {
	...
	var tcRep = {"reason":reason,"isReportId":doTipEx,"type":"1","username":UserName}
	e.stopPropagation();
	$.ajax({
		type: "post",
		url: serviceHOST() + '/tcReport/createTcReport.do',
		dataType: "json",
		data: {tcReport:JSON.stringify(tcRep)},
		success: function(msg) {
			$(".ComplaintsBounced").remove();
			$("#mask").hide();
			$("body").css("overflow","auto");
...
function Level1_comment(msg) {
	str = '<li class="comments-item" data-id="' + msg.id + '"data-name="' + msg.username + '">' +
		...
		'<div class="ui-avatar">';
	//评论者头像
	if (msg.headProtrait == null||msg.headProtrait == "--") {
	...
	str += '</div>' +
		...
		'<span><a href="javascript:;" class="qz_name" data-name=' + msg.username + '>' + (msg.nickname||shieldNumber(msg.username)) + '</a>：</span>' + toFaceImg(msg.content) +
	...
	str += '</div><div class="revert_box" data-id="'+msg.id+'" ></div>' +
		'</div>' +
...
function createComment(productId, pid, username, content, comments_item,type) {
    ...
    $.ajax({
        ...
        success: function (msg) {
            if (msg.status == 0) {
                $(".chitchatBox .p_input .Postcomment").val("");
                $(".chitchatBox .import .Postcomment").val("");
                var mssg = msg.data;
                stairComment(mssg, comments_item,type);
                friendlyMessage("评论成功");
                $(".chitchatBox .revert_box .reply").hide();
                publish = "发表";
            } else if (msg.status == -3) {
                getToken();
            }
...
function stairComment(mssg, comments_item,type) {
	console.log(mssg)
    var str = '<li class="comments-item" data-id="'+mssg.id+'" data-name="">' +
        '<div class="comments-item-bd">' +
        '<div class="ui-avatar">';
    //评论者头像
    if (UserImg == "") {
			...
			if (k < 20) {
				...
				if (commentlist[k].parentCommentNo == null || commentlist[k].parentCommentNo == "") {
					...
					if (commentlist[k].list) {
						str += TwoList(commentlist[k].list);
					}
...
function TheArticleShowUs(mssg, imagepath) {
	...
	if (imagepaths != 0) {
		...
		if (mssg.tcProduct.topictype == 8) { //转发展示
			mssg.tcProduct.content = JSON.parse(mssg.tcProduct.content);
			var url = mssg.tcProduct.content.shareImageUrl;
			var ImgU = "";
			if(window.location.href.indexOf("searchresult.html")>-1){
				str += '<li class="specialOn specialOn_1" data-Surl="' +  mssg.tcProduct.content.shareImageUrl + '" data-conType="' + mssg.tcProduct.content.shareType + '" data-content="' + mssg.topic.content.shareTitle + '" class="Forwardpage" data-page="' + mssg.topic.content.shareUrl + '">' +
				'<a class="clearfix" href="' + mssg.tcProduct.content.shareUrl + '">';
			}else{
				str += '<li class="specialOn specialOn_2" data-Surl="' +  mssg.tcProduct.content.shareImageUrl + '" data-conType="' + mssg.tcProduct.content.shareType + '" data-content="' + mssg.topic.content.shareTitle + '" class="Forwardpage" data-page="' + mssg.topic.content.shareUrl + '">' +
				'<a class="clearfix" href="' + mssg.tcProduct.content.shareUrl + '">';
			};
			
			if (mssg.tcProduct.content.shareTitle != undefined && mssg.tcProduct.content.shareTitle != "undefined") {
				if (mssg.tcProduct.content.shareTitle.length > 40) {
					var shareTitle = mssg.tcProduct.content.shareTitle.substring(0, 40) + "...";
				} else {
					var shareTitle = mssg.tcProduct.content.shareTitle;
				}
			}
	...
	if(mssg.tcProduct.location.split("|")[0].slice(-1)=="市"&&mssg.tcProduct.location.split("|")[1].slice(-1)=="市"){
		var tcLocation = mssg.tcProduct.location.replace(mssg.tcProduct.location.split("|")[0]+"|","").replace("|","-").replace("|","-").replace("|","-")
	}else{
		var tcLocation = mssg.tcProduct.location.replace("|","-").replace("|","-").replace("|","-")
	}
	str +='<div class="urban_location">' +tcLocation+"<span>|</span>"+mssg.tcProduct.productType+ '</div>'
	return str;
...
function DetermineWhetherPraise(mssg, clickMaplist, type) {
	...
	//用户文章发布位置
//	if (mssg.tcProduct.address != "" && mssg.tcProduct.address != "null" && mssg.tcProduct.address != null) {
//		str += '<div class = "location"><p> ' + mssg.tcProduct.address + ' <span> </span></p></div>';
//	}
...
//	}
	if(getCookie("username")){
		if(mssg.user.username ==getCookie("username")){
			str +='<li>'+
			'<a class="again_edit" href="/shangcheng/publish/index.html?productId='+mssg.tcProduct.id+'"><span>去编辑</span></a>'+
			'</li>'+
			'<li class="offShelves" style="padding-left: 0;" proid="'+mssg.tcProduct.id+'"><a class="commodityFrame js_urbanCommodityFrame" href="javascript:;"><span>商品下架</span></a>'+
			'</li>'
		}else{
			if(mssg.user.imgforheadlist && mssg.user.imgforheadlist[0]) {
			str += '<li class="urban_rapidContact"><a class="fast_contact ' + no_login + '" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="' + ImgHOST() + mssg.user.imgforheadlist[0].imagepath + '" data-myindustry="' + mssg.user.myindustry + '">快速联系</a></li>';
		} else {
			str += '<li class="urban_rapidContact"><a class="fast_contact" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="/img/first.png" data-myindustry="' + mssg.user.myindustry + '">快速联系</a></li>';
		}
	//	str += '<li  class="urban_rapidContact"  data-type="chat" data-off="1" data-magicno="'+mssg.user.magicno+'" data-oldname="'+mssg.user.username+'" data-myindustry="'+mssg.user.myindustry+'"><a href="javascript:;"><span class="nickname" style="display:none;">'+mssg.user.nickname+'</span><img style="display:none;" src="http://img.quanzinet.com/files/TX-a3092289-8c37-49ee-8c07-07d45746303d"><span>快速联系</span></a></li>';
		//收藏
		if (mssg.favorite) {
			str += '<li><a href="javascript:;" class="collection collection_Cancel" data-urbantype="'+mssg.tcProduct.type+'" data-urbanproid="'+mssg.tcProduct.id+'" data-urbanusername="'+mssg.tcProduct.username+'"><i></i><span>取消收藏</span></a></li><li>|</li>';
		} else {
			str += '<li><a class="collection collection_post ' + no_login + '" href="javascript:;" data-urbantype="'+mssg.tcProduct.type+'" data-urbanproid="'+mssg.tcProduct.id+'" data-urbanusername="'+mssg.tcProduct.username+'"><i></i><span>收藏</span></a></li><li>|</li>';
		}
		//联系方式 
		if(mssg.tcProduct.otMobile){var otmobile = '<li>'+mssg.tcProduct.otMobile+'</li>'}else{var otmobile = ""}
		if(mssg.tcProduct.mobile){var tcmobile = '<li>'+mssg.tcProduct.mobile+'</li>'}else{var tcmobile = ""}
		str +='<li class="urban_contact">' +
			'<a href="javascript:;"><i></i>联系方式</a><ul class="urban_contact_list"><li>'+mssg.tcProduct.contacts+'</li>'+tcmobile+otmobile+'</ul>' +
			'</li><li>|</li>'
		}
	...
	//	str += '<li  class="urban_rapidContact"  data-type="chat" data-off="1" data-magicno="'+mssg.user.magicno+'" data-oldname="'+mssg.user.username+'" data-myindustry="'+mssg.user.myindustry+'"><a href="javascript:;"><span class="nickname" style="display:none;">'+mssg.user.nickname+'</span><img style="display:none;" src="http://img.quanzinet.com/files/TX-a3092289-8c37-49ee-8c07-07d45746303d"><span>快速联系</span></a></li>';
		...
		//联系方式 
		str +='<li class="urban_contact">' +
			'<a href="javascript:;"><i></i>联系方式</a><ul class="urban_contact_list"><li>'+mssg.user.nickname+'</li><li>'+mssg.user.username+'</li></ul>' +
			'</li><li>|</li>'
	}
...
//	//收藏
//	if (mssg.favorite) {
//		str += '<li><a href="javascript:;" class="collection collection_Cancel" data-urbantype="'+mssg.tcProduct.type+'" data-urbanproid="'+mssg.tcProduct.id+'" data-urbanusername="'+mssg.tcProduct.username+'"><i></i><span>取消收藏</span></a></li>';
//	} else {
//		str += '<li><a class="collection collection_post ' + no_login + '" href="javascript:;" data-urbantype="'+mssg.tcProduct.type+'" data-urbanproid="'+mssg.tcProduct.id+'" data-urbanusername="'+mssg.tcProduct.username+'"><i></i><span>收藏</span></a></li>';
//	}

	//更多
//	str += '<li>|</li><li class=""><a class="urban_more' + no_login + '" href="javascript:;"><i></i><span>更多</span></a></li>';
	if(mssg.user.username != getCookie("username")){
		str += '<li>|</li><div class="urban_shield urban_more"><a class="fl_menu" href="javascript:;"><i></i>更多</a><ul class="fl_menu_list"><li class="complaint"><i></i>举报</li><ul></div>'
	}
	str += '<br class="clear"/>' +
		'</ul>';
	//点赞人头像		
...
//商品下架
$(document).on("click", ".js_urbanCommodityFrame", function() {
...
$(document).on("click", ".content_items .collection_post", function() {
	...
	if (UserName) {
		if ($('.joinUs').attr('data-status') == 0) {
			...
			createCollections(_this);
		}
	}
})

// 添加帖子收藏
function createCollections(_this) {
	$.ajax({
		type: "post",
		url: serviceHOST() + "/tcFavorite/createTcFavorite.do",
		dataType: "json",
		headers: {
			"token": qz_token()
		},
		data: {
			type: _this.attr("data-urbantype"),
			username: getCookie("username"),
			productId: _this.attr("data-urbanproid"),
		},
		success: function(msg) {
			if (msg.status == 0) {
...
$(document).on("click", ".content_items .collection_Cancel", function() {
	...
	// } else {
		$.im.confirm("确定要取消收藏吗？", function() {
			$.ajax({
				type: "post",
				url: serviceHOST() + "/tcFavorite/deleteTcFavorite.do",
				dataType: "json",
				data: {
					username: getCookie("username"),
					productId: _this.attr("data-urbanproid"),
...
if (UserName) {
    ...
    $(document).on("click", ".content_items .review_1 .publish", function () {
        if (getCookie("username")) {
            ...
            var id=$(this).parents(".content_items").attr("data-id");
            ...
            var comments_item =$(this).parents(".review_1").siblings(".chitchatBox").find(".stairComment");
...
function PostCommentsContent(mssg, commentlist, nums, str, cl) {
	...
	if (commentlist != null) {
		if (mssg.tcLeaveWordList.length > 20) {
			console.log(mssg)
			var ThereAre = mssg.tcLeaveWordList.length - 20;
			str += '</ul><div class="To_view_more"><a href="/center/shopDetails.html?id=' + mssg.tcProduct.id + '&from=' + mssg.user.username + '" class="view_more">后面还有' + ThereAre + '条点击查看&gt;</a></div></div>';
		} else {
...
function PostDynamicUserDetails(mssg,type) {
	var videoUrl = mssg.tcProduct.videourl?2:0;
	var str = "";
	str = '<div class="content_items" data-type='+videoUrl+' data-prdtype='+mssg.tcProduct.type+'  data-type=' + mssg.tcProduct.type + ' data-id=' + mssg.tcProduct.id + ' data-name=' + mssg.user.username + '>' +
	...
	str += '<dd class="message_time"><span class="message_professinal">' + mssg.user.myindustry + '</span><span>' + formatTime(mssg.tcProduct.createtime) + '</span></dd>' +
		'</dl>';
...
function PostDynamicTitlePrice(mssg){
	if(getURIArgs("word")=="undefined"){
		var word = "";
	}else{
		var word = getURIArgs("word");
	}
	if(mssg.tcProduct.type==1){
		var urban_price = peiceShowAllAry[mssg.tcProduct.originalPrice]||"面议";
	}else if(mssg.tcProduct.price==0){
		var urban_price = "面议"
	}else{
		if(mssg.tcProduct.type==3){
			var urban_price = mssg.tcProduct.price+"元"
		}else{
			var urban_price = mssg.tcProduct.price+"元/月"
		}
	}
	if(mssg.tcProduct.title.indexOf(word) > -1){
		var mssgTitle = mssg.tcProduct.title.replace(word, "<span style='color:#ff8a00;'>" + word + "</span>");
	}else{
		var mssgTitle = mssg.tcProduct.title;
	}
	var str = '<p class="urban_title">'+mssgTitle+'</p>'+
			'<div class="urban_price">'+urban_price+'</div>'
	return str;
}
...
function DynamicPostArticles(mssg) {
	...
	if (mssg.tcProduct.type == 8) { //转发		
		var msg = JSON.parse(mssg.tcProduct.content);
		var content = '';
		...
		if (mssg.tcProduct.content != undefined && mssg.tcProduct.content.indexOf("\n") > -1) { //判断有否有回车符  有则换行
			var topicContent = mssg.tcProduct.content.replace(/\n/g, "</br>");
		} else {
			var topicContent = mssg.tcProduct.content;
		}
		str += '<div class="article_content" data-content="' + mssg.tcProduct.content + '">' + toFaceImg(topicContent) + '</div></div>';
	}

	str += '<div class="onDisplay"></div>' +
		'<div class="pictureShow photoblock-many">' +
		'<ul>';
	if (mssg.tcProduct.videourl) {
		str += '<div class="article_video">' + addVideo(ImgHOST() + mssg.tcProduct.videourl, mssg.tcProduct.id, ImgHOST() + mssg.tcProduct.videourl + "-start") +
			'</div>';
	}
	return str;
}
...
function SideColumnMyCircle(msg) {
	...
	for (var i = 0; i < dataLength; i++) {
		...
		tpl+='</span></p><div>\
		<span><i class="item_01">' + (mssg.usercount/10000).toFixed(2) + '</i>万人加入</span><span>|</span><span>\
		<i class="item_02">' + (mssg.activecount/10000).toFixed(2) + '</i>万人活跃</span></div>\
		</div>\
		</a>\
		</li>';
	}
