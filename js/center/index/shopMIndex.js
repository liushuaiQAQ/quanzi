...
function DetermineWhetherPraiseShop(mssg, type) {
    var str = "";
    str += '</div><div class = "location"><p class="identify">'+mssg.tcProduct.location+'</p></div>';
    if(type==1){
        str+='<div class="qz_handle qz_handleShop">' +
            '<ul class="qz_row_line">' +
            '<li>'+
            '<a class="again_edit" href="/shangcheng/publish/index.html?productId='+mssg.tcProduct.id+'" ><i></i><span>去编辑</span></a>'+
            '</li>'+
            '<li>|</li>'+
            '<li>';
            if(!mssg.tcProduct.isSale) {
                str+='<a proId="'+mssg.tcProduct.id+'" class="againCommodityFrame js_againCommodityFrame" href="javascript:;"><i></i><span>重新上架</span></a>';
            }else{
                str+='<a  proId="'+mssg.tcProduct.id+'" class="commodityFrame js_commodityFrame" href="javascript:;"><i></i><span>商品下架</span></a>';
            }
            str+='</li>'+
            '<li>|</li>'+
            '<li>';
            if(getURIArgs("type") == 1){
                str+='<a class="thoroughDelete" proId="'+mssg.tcProduct.id+'" href="javascript:;"><i></i>彻底删除</a>';
            }else{
        ...
        str+='<div class="qz_handle qz_handleShop">' +
            ...
            if(mssg.user.imgforheadlist && mssg.user.imgforheadlist[0]) {
                str += '<a class="fast_contact ' + no_login + '" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="' + ImgHOST() + mssg.user.imgforheadlist[0].imagepath + '" data-myindustry="' + mssg.user.myindustry + '"><span>快速联系</span></a>';
            } else {
                str += '<a class="fast_contact" href="javascript:;" data-magicno="' + mssg.user.id + '" data-nick="' + (mssg.user.nickname || shieldNumber(mssg.user.username)) + '" data-oldname="' + mssg.user.username + '@imsvrdell1" data-img="/img/first.png" data-myindustry="' + mssg.user.myindustry + '"><span>快速联系</span></a>';
            }
            str+='</li>';
            //收藏
            if (mssg.favorite) {
                str += '<li><a proId="'+mssg.tcProduct.id+'" data_type="'+mssg.tcProduct.type+'" href="javascript:;" class="collection collection_shopCancel"><i></i><span>取消收藏</span></a></li>';
            } else {
                str += '<li><a proId="'+mssg.tcProduct.id+'" data_type="'+mssg.tcProduct.type+'" class="collection collection_post_shop ' + no_login + '" href="javascript:;"><i></i><span>收藏</span></a></li>';
            }
            str+='<li>|</li>'+
            '<li>'+
            ...
            '<ul>'+
            '<li><i></i><span>'+mssg.tcProduct.contacts+'</span></li>' +
            '<li><i></i><span>'+mssg.tcProduct.mobile+'</span></li>';
            if(mssg.tcProduct.otMobile){
                str+='<li><i></i><span>'+mssg.tcProduct.otMobile+'</span></li>';
            }
            str+='</ul>'+
            '</div>'+
            '</li>'+
...
function TheArticleShowUsShop(mssg, imagepath) {
    var imagepaths = imagepath.length;
    var str = "";
    var tcProductShop=mssg.tcProduct;
    ...
    if (imagepaths != 0) {
        ...
        if (tcProductShop.topictype == 8) { //转发展示
            tcProductShop.content = JSON.parse(tcProductShop.content);
            var url = tcProductShop.content.shareImageUrl;
            var ImgU = "";
            if(window.location.href.indexOf("searchresult.html")>-1){
                str += '<li class="specialOn specialOn_1" data-Surl="' +  tcProductShop.content.shareImageUrl + '" data-conType="' + tcProductShop.content.shareType + '" data-content="' + tcProductShop.content.shareTitle + '" class="Forwardpage" data-page="' + tcProductShop.content.shareUrl + '">' +
                    '<a class="clearfix" href="' + tcProductShop.content.shareUrl + '">';
            }else{
                str += '<li class="specialOn specialOn_2" data-Surl="' +  tcProductShop.content.shareImageUrl + '" data-conType="' + tcProductShop.content.shareType + '" data-content="' + tcProductShop.content.shareTitle + '" class="Forwardpage" data-page="' + tcProductShop.content.shareUrl + '">' +
                    '<a class="clearfix" href="' + tcProductShop.content.shareUrl + '">';
            };

            if (tcProductShop.content.shareTitle != undefined && tcProductShop.content.shareTitle != "undefined") {
                if (tcProductShop.content.shareTitle.length > 40) {
                    var shareTitle = tcProductShop.content.shareTitle.substring(0, 40) + "...";
                } else {
                    var shareTitle = tcProductShop.content.shareTitle;
                }
            }
            if (url != undefined && url != "") {
                ...
                if (tcProductShop.content.shareImageType == 1) {
...
function jobSalaryShowFun(mssg){
    var str="";
    str='<div class="jobSalaryShow">'+
        '<p class="title"><a href="/center/shopDetails.html?id='+mssg.tcProduct.id+'" >'+mssg.tcProduct.title+'</a></p>'+
        '<p class="salary">';
        if(mssg.tcProduct.type==1){
            str+=peiceShowAllAry[mssg.tcProduct.originalPrice]||"面议";
        }else{
            str+=(mssg.tcProduct.price!=0)?mssg.tcProduct.price+"元/月":"面议";
        }
        str+='</p>'+
        '</div>';
    return str;
...
$(document).on("click", ".content_items .collection_post_shop", function() {
    ...
    var allType=$(this).attr("data_type");
    var _this = $(this);
    if (UserName) {
        if ($('.joinUs').attr('data-status') == 0) {
            warningMessage('请先加入圈子');
        } else {
            collectCanFun(1,DeletePosts, _this,allType);
        }
    }
});
...
//收藏与取消收藏
function  collectCanFun(type,id,_this,allType){
    var data="";
    if(type==1){
...
//        str += '</ul><div class="clear"></div></div>' + picShow + '<div class="imgs">' + picStr + '<div class="clear"></div></div></div>';
//    } else {
//        if (mssg.topic.topictype == 8) { //转发展示
...
//            };
//
//            if (mssg.topic.content.shareTitle != undefined && mssg.topic.content.shareTitle != "undefined") {
...
function PostDynamicUserDetails(mssg,type) {
    ...
    str = '<div class="content_items" data-type=' + topictypeShop + ' data-prdtype="'+mssg.tcProduct.type+'" data-id=' + mssg.tcProduct.id + ' data-name=' + mssg.user.username + '>' +
    ...
    if(mssg.user.username != UserName && mssg.isCollections != 1 && !type){
        str += '<div class="shield" style="display:none;"><a class="fl_menu" href="javascript:;"></a><ul class="fl_menu_list"><li class="shield_dt">屏蔽此动态</li><li class="shield_dt_all">不看他动态</li><li class="complaint">投诉</li><ul></div>'
    }
...
function DynamicPostArticles(mssg) {
    var str = "";
    str += '<div class="article" style="padding-bottom:10px;" >';
    if (mssg.tcProduct.topictype == 8) { //转发
    ...
    //topictype==2  动态为视频
    if (mssg.tcProduct.videourl) {
        str += '<div class="article_video">' + addVideo(ImgHOST() + mssg.tcProduct.videourl, mssg.tcProduct.id, ImgHOST() + mssg.tcProduct.videourl + "-start-s") +
            '</div>';
    }
    return str;
}
